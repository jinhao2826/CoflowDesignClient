...
//int dscp[] = {0, 0, 8, 16, 24, 32, 40, 48, 56};
unsigned int dscp[] = {0x00, 0x00, 0x20, 0x40, 0x60, 0x80, 0xa0, 0xc0, 0xe0};
struct pureackinfo *pureackinfo_header = NULL;

spinlock_t coflow_lock = __SPIN_LOCK_UNLOCKED(coflow_lock);
/*end*/
...
/*haojin*/
	...
	struct pureackinfo *info;
	struct pureackinfo	*previous;
/*end*/
...
/*haojin*/
//	struct iphdr *iph;
	struct pureackinfo *p = NULL;
	struct pureackinfo *prev = NULL;
	struct pureackinfo *pureack = NULL;
...
//	printk(KERN_ALERT "src port: %x dest port: %x\n", th->source, th->dest);
...
//			printk(KERN_ALERT "slow_path1\n");
			/* If PAWS failed, check it more carefully in slow path */
			if ((s32)(tp->rx_opt.rcv_tsval - tp->rx_opt.ts_recent) < 0)
				goto slow_path;
//			printk(KERN_ALERT "slow_path2\n");
		...
		if (len <= tcp_header_len) {
			...
			if (len == tcp_header_len) {
				 ...
				 * on entry.
				 */
//				printk(KERN_ALERT "receive ack in\n");
...
slow_path:
//	printk(KERN_ALERT "slow_path:source port: %x  dest port %x\n", th->source, th->dest);
...
//							printk(KERN_ALERT "update res1: %x\n", th->res1);
			...
			if (p == NULL){
					...
					if (pureack = kmalloc(sizeof(struct pureackinfo), GFP_KERNEL)){
						...
						pureack->res1 = th->res1;
						pureack->next = NULL;
//						printk(KERN_ALERT "step5:pureackinfo create: dstport: %x	srcport: %x\n", pureack->dstport, pureack->srcport);
					}

					if (pureackinfo_header == NULL) {
			
						pureackinfo_header = pureack;
//						printk(KERN_ALERT "step5:add in the header\n");	
			
					} else {
			
						prev->next = pureack;
//						printk(KERN_ALERT "step5:add in the list\n");	
					}
